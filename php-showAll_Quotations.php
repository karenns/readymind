...
/* AJAX */
	...
	$json_quotation = $quotation -> get_All_Quotations();
	...
	if ($json_quotation){
		foreach ($json["Result"] as $field) {	
			...
			$last_revision_letter = $revision-> get_LastrevisionLetter($quotation_number);
			...
			//Get company name
			$customer_company = $customer -> get_Customer_company($customer_id);
